...
def wfpt_gen(v_mean, a, w_mode, w_std=0.0,
             ...
             err=.0001, nsamp=1000, trange=None, need_prob_no_resp = False):
    ...
    if need_prob_no_resp:
        p_non_resp = np.clip(1 - (likes*dx).sum(), 0, 1)
        # p_non_resp = 1 - (likes*dx).sum()
        # print(p_non_resp, np.log(p_non_resp))
        return p_non_resp
